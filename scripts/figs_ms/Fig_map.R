...
lookup <- lookup_raw %>%
  ...
  select(site, sp, species, count, lat, lon, col) %>%
...
## Species labels:
# sp_labs <- c('sp. #3', expression(italic(macarthurii)),
#              expression(italic(lehilahytsara)), expression(italic(mittermeieri)),
#              expression(italic(simmonsi)))
sp_labs <- c(msp3 = "<b><i style='color:#D55E00'>sp. #3</i></b>",
             mmac = "<b><i style='color:#e6da3e'>macarthurii</i></b>",
             mleh = "<b><i style='color:#0072B2'>lehilahytsara</i></b>",
             mmit = "<b><i style='color:#56B4E9'>mittermeieri</i></b>",
             msim = "<b><i style='color:#999999'>simmonsi</i></b>")
...
p_msp3 <- ggplot() +
  ...
  geom_sf(data = filter(veg_all, vegetation == 'humid_low'),
           fill = veg_cols[1], alpha = 0.2, color = NA, lwd = 0) +
  geom_sf(data = filter(veg_all, vegetation == 'humid_mont'),
           fill = veg_cols[2], alpha = 0.2, color = NA, lwd = 0) +
  geom_sf(data = rivers, fill = 'skyblue3', colour = 'skyblue3',
          alpha = 0.8, lwd = 0.7) +
...
figfile <- paste0(figfile_base, '.pdf')
